---
name: au-task2
description: Package an existing converted folder into OUTPUT.LST and dir_music without converting audio. Invoke explicitly with /au-task2.
...
3. 调用 `check_audio_environment`。本模式要求 pRFiles、packres.exe、
   new_packres.bat和test_dir可用；转换工具或FFmpeg缺失不影响本模式。
...
`test_dir`，执行 `new_packres.bat` 并校验 `dir_music`。

5. 保存 `job_id`，使用 `get_audio_workflow_status` 持续查询至终态。用户
   要求取消时调用 `cancel_audio_workflow`。
6. 成功后列出 `OUTPUT.LST` 和 `dir_music` 完整路径；失败时给出
   `error` 与最后相关日志。
