...
生成到 `processed` 的文件名会删除所有空格、Tab 等空白字符，但不会
重命名原始录音。例如：
...
模式2/3读取已有音频、模式4读取旧 `converted` 时也会在进入下一阶段前
去除文件名空白。若去空格后重名，自动添加 `_1`、`_2`，不会覆盖文件。
因此转换文件、`OUTPUT.LST` 和 `test_dir` 中的名称保持一致。
...
```json
{
  ...
  "packres_input_directory": "C:\\实际目录\\test_dir"
}
```
